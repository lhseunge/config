...
prompt_context() {
  if [[ "$USER" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    if [[ "$USER" == "seung" ]]; then
      ...
      prompt_segment black default "%(!.%{%F{yellow}%}.)K2S LHS"
    fi
  fi
}
...
prompt_command() {
  if [[ -n $CURRENT_BG ]]; then
    ...
    # command prompt open
    # echo -n "%{%K{white}%F{black}%}$SEGMENT_SEPARATOR COMMAND "
    echo -n "%{%K{white}%F{black}%} COMMAND "

    # command prompt close
#    echo -n "%{%K{black}%F{white}%}$SEGMENT_SEPARATOR"
...
prompt_status() {
  ...
  [[ $RETVAL -ne 0 ]] && symbols="%}✘"
  [[ $RETVAL -ne 0 ]] && promptColor="red"
  [[ $UID -eq 0 ]] && symbols="⚡"
  [[ $UID -eq 0 ]] && promptColor="yellow"
  [[ $(jobs -l | wc -l) -gt 0 ]] && symbols="⚙"
  [[ $(jobs -l | wc -l) -gt 0 ]] && promptColors="cyan"
 
  echo -n "%{%K{${promptColor}}%F{white}%}$SEGMENT_SEPARATOR $symbols %{%K{black}%F{${promptColor}}%}$SEGMENT_SEPARATOR"
}
